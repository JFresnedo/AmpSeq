...
##v2
unless (getopts("s:k:i:o:l:b:c:r:a:e:w:v:t:m:d:x:j:h", \%opts))
{
        printhelp();
...
our $trimmomatic_adapter = "/Programs/Trimmomatic-0.36/adapters/NexteraPE-PE.fa";
our $fastx_trimmer  = "fastx_trimmer -Q33 -l ";
...
if ($opts{"e"}) 
{
	...
	{
		printhelp();
		print "Error: -e must be a number!";
	}
}

#####trimmomatic parameter
my $quality_trimmer = 10;
...
if ($opts{"d"}) 
{
	...
	{
		printhelp();
		print "Error: -d must be two numbers separated by ':'. E.g. 10:50. They are: quality score:minimum_read_length!\n";
		exit;
	}
}
...
if ($opts{"v"}) 
{
		...
		print "Error: -v is not supported under single end mode!\n";
		exit;
}
...
if ($opts{"w"}) 
{
		print "Error: -w is not supported under single end mode!\n";
		exit;
}
...
if (((! $opts{"i"})) || (($opts{"i"}) && (! ($opts{"i"}=~/1/))))
{
	...
	LOOPi1:foreach my $sample (@samples) 
	{
		...
		my $trimmomatic_command = "$trimmomatic_cmd SE -phred33 $file1  $trimmdir/$sample.r1.fastq ".
		...
		if ($trim_out =~/(Input Reads.+)/i)
		{
			$1=~/Input Reads:\s+(\d+)\s+Surviving:\s+(\d+)\s+\(([0-9.]+)/i;
		...
		else
		{
			print LOG "$trim_out\n";
			close LOG;
			print "Error: Trimmomatic on $file1 failed!\n";
...
if (((! $opts{"i"})) || (($opts{"i"}) && (! ($opts{"i"}=~/2/))))
{
	...
	LOOPi2:foreach my $sample (@samples) 
	{
		...
		{
			system ("$fastx_trimmer $concatenate_read_length -i $file1 -o $assembledir/$sample.extendedFrags.fastq");
		}
		else
		{
			system ("cp $file1 $assembledir/$sample.extendedFrags.fastq");
		}
	...
	LOOPi7:foreach my $gene (@genes)
	{
		...
		LOOPi7_1:foreach  my $sample(@samples) 
		{
			if (exists $tbt{$sample}) 
			{
				...
				if ($haplotype2counts{$a1}<=$mindepth) 
				{
...
if (((! $opts{"i"})) || (($opts{"i"}) && (! ($opts{"i"}=~/8/))))
{
	...
	LOOPi8:foreach my $gene (@genes)
	{
		...
		print OUT ">$gene\n$refseqstr\n";
		close OUT;
		system ("bwa_cmd index $samplegene2fastadir/ref_${gene}.fasta &>> $bwalog");
		...
		system ("java -jar ${picard_path}SamFormatConverter.jar INPUT=$samplegene2fastadir/${gene}.sam OUTPUT=$samplegene2fastadir/${gene}.bam QUIET=true VERBOSITY=ERROR ");
		system ("java -jar ${picard_path}SortSam.jar INPUT=$samplegene2fastadir/${gene}.bam OUTPUT=$samplegene2fastadir/${gene}.sorted.bam SORT_ORDER=coordinate QUIET=true VERBOSITY=ERROR");
		system ("java -jar ${picard_path}CleanSam.jar INPUT=$samplegene2fastadir/${gene}.sorted.bam OUTPUT=$samplegene2fastadir/${gene}.clean.bam QUIET=true VERBOSITY=ERROR");
		system ("java -jar ${picard_path}BuildBamIndex.jar INPUT=$samplegene2fastadir/${gene}.clean.bam QUIET=true VERBOSITY=ERROR");
		system ("$samtools_cmd faidx $samplegene2fastadir/ref_${gene}.fasta");
		system ("java -jar ${picard_path}CreateSequenceDictionary.jar REFERENCE=$samplegene2fastadir/ref_${gene}.fasta OUTPUT=$samplegene2fastadir/ref_${gene}.dict QUIET=true VERBOSITY=ERROR");
...
sub printhelp
{
	...
	print "-d: parameter for trimming reads, using 2 numbers separated by ':'. Default: 10:50. They are: quality score:minimum_read_length\n";
	...
	print "-l: Truncate read to specified length\n";
	...
	print "-e: minimm read depth to call an allele, default 2\n";
	...
	print "2. Contiging/Concatenating read pairs.\n";
